...
const PHOTO_PROMPTS = [
  `Alright — almost done 🙌\nLast thing: 4 quick full-body photos. Stand at arm's length from the camera, natural posture. They're private — only you and your coach will see them.\nFirst one: facing the camera straight on.`,
  `Got it 📸 Now a back view — turn around.`,
  `Nice. Left side now.`,
  `Last one — right side and you're done!`,
];
...
async function finish(ctx) {
  ...
  await sendBubbles(ctx, `That's everything${name} 🎉\nGive me a moment to put this together for your coach...`);
  ...
  await sendBubbles(ctx,
    `You're all set 🙌\nSeriously — most people dread this part but you got through it. Your coach is going to have everything they need to build you a proper plan.\nThey'll reach out very soon. If you need to update anything before then, just tap /start.`
  );
}
...
bot.command('start', async (ctx) => {
  resetSession(ctx);
  await sendBubbles(ctx,
    `Hey! I'm ${BOT_NAME} from the Mealzy team 👋\nBefore your coach gets started, I just need to get to know you a bit — your lifestyle, goals, what's worked and what hasn't.\nThink of this as a quick chat, not a form. Let's start easy — what should I call you?`
  );
});
